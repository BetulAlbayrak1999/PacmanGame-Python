...
class Main:
    def draw(self):
        ...
        if Call_coin.score() == 0:
            # draw pacman
            glPushMatrix()
            Call_pacman.drawPacman(7.6, 0.4, 1)
            glPopMatrix()
            glPushMatrix()
...
def main():
    ...
    glutKeyboardFunc(pacman.PacMan.key_pressed)
    glutSpecialFunc(pacman.PacMan.key_pressed_special)
    glutSpecialUpFunc(pacman.PacMan.key_pressed_special_up)
    start_game.InitGL()
    glutMainLoop()
